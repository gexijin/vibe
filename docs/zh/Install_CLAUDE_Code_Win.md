...
Claude Code 是一款运行在终端里的 AI 助手，能帮你编写、调试和理解代码。按照本指南，几分钟就能在 Windows 上完成原生安装并上手使用。如果想要更进阶的环境，指南末尾还介绍了可选的 WSL2 安装方法。

## 核心概念

- **PowerShell** - Windows 自带的命令行工具，本指南用它来安装和运行 Claude Code
- **Git for Windows** - 一款免费工具，自带 **Git Bash**，也就是 Claude Code 执行命令时使用的 shell
- **WSL（Windows Subsystem for Linux）** - Windows 的一项可选功能，能在 Windows 里运行一个真正的 Linux 系统；指南末尾的进阶设置会用到它
...
建议安装 Git for Windows，但它**并非必需**。它会提供 **Git Bash**，也就是 Claude Code 执行命令时使用的 shell。

- 访问 [git-scm.com/download/win](https://git-scm.com/download/win)
- 页面通常会自动开始下载，下载完成后打开安装程序
- 在安装程序里一路点击 **Next**，保持默认选项即可
...
- 点击 **Windows 开始按钮**
- 输入 `PowerShell`，点击 **Windows PowerShell**
- 在 PowerShell 窗口里输入：
   ```
...
- 在 PowerShell 窗口里输入：
   ...
   claude
   ```
- Claude 会尝试打开浏览器。如果没能自动打开，按住 **Ctrl** 点击那一长串网址即可在浏览器中打开；也可以把网址复制到浏览器里打开
- 登录 Claude.ai 账户（用 Chrome 的话可能会自动登录）
- 点击 **Authorize**
- 看到一长串代码后，点击 **Copy Code**
- 回到终端窗口
- 在终端里粘贴：**右键单击**后选择**粘贴**（或按 **Ctrl+Shift+V**）
- 随后会看到成功提示
- 按照提示完成后续设置

下面几种基于 API 的方式都在 **PowerShell** 里完成，用到的 `setx` 命令会把设置永久保存下来。运行 `setx` 之后，记得**先关闭再重新打开 PowerShell**，让设置生效，然后再启动 `claude`。
...
- 在 PowerShell 里输入：
   ...
   ```
   把 `your-api-key-here` 换成你自己的 API 密钥
- 关闭并重新打开 PowerShell，让设置生效
- 启动 Claude Code：
   ```
   claude
   ```
...
对大多数人来说，上面的原生设置已经够用了。不过 Claude Code 在 Linux 下会跑得更顺：WSL2 能在 Windows 里运行一个真正的 Linux 系统，不仅带来 **Bash 工具沙盒**（一项安全特性），也能和各种 Linux 工具链配合得更好。它配置起来更花时间，还要重启电脑，所以只有在你确实想要完整 Linux 体验时再做这一步。
...
- 重启电脑
- 开机过程中按下进入 BIOS 的按键（通常是 **F2**、**F10**、**Del** 或 **Esc**，具体取决于电脑品牌）
- 找到"Virtualization Technology"、"Intel VT-x"、"AMD-V"或"SVM Mode"等相关选项
- 将其开启
- 保存并退出 BIOS（通常按 **F10**）
...
- 等待出现 `Enter new UNIX username:` 这行提示
- 输入一个用户名（只用小写字母和数字，不要有空格），例如 `john`
- 接着会出现 `New password:`
- 输入一个简单的密码（输入时屏幕不会显示任何字符，这是正常现象）
- 按提示再输入一次相同的密码
...
- 输入以下命令验证：
   ...
   claude --version
   ```
- 在 Ubuntu 终端里输入 `claude`，按浏览器登录流程连接账户（和步骤 3 的选项 A 一样）。如果改用 API 密钥，则用 `export ANTHROPIC_API_KEY="your-api-key-here"` 来设置（WSL 里用的是 Linux 命令，而不是 `setx`）
- 要在 WSL 中打开 Windows 上的项目文件夹：
   ```
   cd /mnt/c/Users/Username/Documents/test_claude
   ```
   把 `Username` 换成你实际的 Windows 用户名。
