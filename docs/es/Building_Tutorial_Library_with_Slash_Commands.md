...
No me senté a diseñar comandos slash desde cero. En cambio, creé los primeros tutoriales manualmente a través de prompts interactivos con Claude, refiné el proceso a través de la iteración, y luego le pedí a Claude que codificara el proceso funcional en un comando slash.

Este es exactamente el patrón descrito en mi [tutorial de artículo de investigación](./Writing_Research_Paper_Claude_Code): pasar por el flujo de trabajo manualmente, luego en el paso final, pedirle a Claude que lo guarde como un comando reutilizable.
...
3. **Iteración**: Reviso el plan, le pido a Claude que revise varias veces hasta estar satisfecho
4. **Escritura**: Después de la aprobación, Claude escribe siguiendo reglas de formato estrictas:
   ...
   - Fecha de creación
5. **Prueba**: Pruebo los pasos yo mismo en una terminal separada, reviso si es necesario
6. **Pulido**: A veces edito manualmente, a veces le pido a Claude que lo haga

El comando impone una estructura consistente en todos los tutoriales. Cada tutorial se siente como si viniera del mismo autor—porque siguieron el mismo proceso sistemático. Generé más de 20 tutoriales cubriendo temas desde operaciones básicas de Git hasta flujos de trabajo avanzados de Docker.

### [`/review-tutorial`](../assets/commands/review-tutorial.md) - El bot de control de calidad

Aquí hay algo notable: simplemente le pedí a Claude que "creara un comando slash para revisar tutoriales". Sin especificaciones detalladas. Claude generó un flujo de trabajo completo de tres fases con más de 30 criterios de calidad organizados en categorías:
...
- **Precisión técnica**: comandos correctos, UI actual, estimaciones de tiempo realistas
- **Calidad de escritura**: gramática, terminología consistente, lenguaje conciso

El comando presenta los hallazgos en un informe estructurado, luego aplica correcciones después de la aprobación. ¿Por qué era necesario esto? Dos razones: Claude no siempre sigue estrictamente las reglas de `/tutorial`, y el comando `/tutorial` en sí evolucionó a medida que construía más tutoriales. El comando de revisión me permite pulir por lotes los tutoriales anteriores para que se ajusten a los últimos estándares.
...
De nuevo, simplemente le pedí a Claude que "creara un comando slash para traducir tutoriales al chino"—sin pautas específicas. Claude generó un flujo de trabajo de seis fases con reglas completas:
...
Los tutoriales evolucionan. Los comandos cambian. Se añaden nuevas secciones. Este comando mantiene las traducciones sincronizadas a través de un flujo de trabajo de cuatro fases:

1. **Leer ambas versiones**: Cargar el original en inglés y la traducción lado a lado
2. **Comparar y actualizar**: Identificar secciones faltantes, contenido desactualizado, comandos o URLs cambiados
3. **Revisar calidad**: Verificar precisión de traducción, calidad del idioma, consistencia de formato
4. **Informar y corregir**: Presentar hallazgos, obtener aprobación, aplicar actualizaciones

La revisión de calidad es exhaustiva—para japonés, verifica frases naturales (no palabra por palabra), nivel de cortesía apropiado (forma です/ます), uso correcto de partículas (は/が, を, に, で), y sin uso excesivo de katakana antinatural. Cuando actualicé los tutoriales en inglés, pude propagar rápidamente los cambios a todas las traducciones manteniendo la calidad del idioma.
...
La combinación de comandos slash para flujos de trabajo estructurados y subagentes para paralelización creó un pipeline de documentación que escaló mucho más allá de lo que el esfuerzo manual podría lograr.

## Los resultados

En menos de dos semanas, creé más de 20 tutoriales en inglés cubriendo temas de principiante a intermedio, 81 tutoriales traducidos en 3 idiomas, calidad consistente a través de procesos de revisión automatizados, y documentación mantenible con herramientas de sincronización para traducciones.
...
- **Empieza manual, luego automatiza.** Haz la tarea manualmente primero, refina el proceso, luego pídele a Claude que lo guarde como un comando.
...
Construir esta biblioteca de tutoriales me enseñó que la automatización no se trata de eliminar a los humanos del proceso—se trata de amplificar el juicio humano. No escribí más de 100 tutoriales manualmente, pero cada uno refleja mis estándares, mi estructura y mi aprobación.

Los comandos slash transformaron Claude Code de un asistente útil en una fábrica de documentación que trabaja bajo mi dirección, mantiene mis estándares y escala a cualquier tamaño de proyecto que necesite.

Si tienes tareas de documentación repetitivas, no sigas haciéndolas manualmente. Construye el comando slash una vez, luego despliégalo docenas o cientos de veces.
...
**P.D.** Esta publicación de blog fue redactada por Claude Code a través de prompts iterativos:
...
2. "Refleja que el comando /tutorial evolucionó primero de prompts interactivos manuales."
3. "Enfatiza mi estilo de tutorial: práctico, enfocado, paso a paso."
4. "Añade que las traducciones al japonés llegaron primero sin comandos slash, y Claude usó 8 subagentes en paralelo."
5. "Añade que Claude escribió los comandos slash de traducción sin pautas específicas, y 25 tutoriales fueron traducidos en 15 minutos usando subagentes."
